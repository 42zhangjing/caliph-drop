...
```text
Finder 图片
   ↓ 拖到 Mac 顶部菜单栏的 Caliph Drop 图标（或超大感应热区）
...
Worker 验证专用 Token
   ↓
R2 保存图片 + D1 自动新增图库记录（支持单张收录或多图合辑）
   ↓
网站图库立刻出现
```

0.4 支持超大状态栏拖拽热区、加大虚线框、精确上传时间显示，以及多图拖入时的智能确认（分别独立收录 vs 合并为一条图集）。弹窗可通过点击外部区域或按 `Escape` 收起；上传在后台执行，菜单栏图标会显示上传中、成功或失败状态。
